...
| `10000` | The same server in the Render deployment, via `AFFINE_SERVER_PORT` | as above | `render.yaml:23-24` |
...
(`.docker/selfhost/compose.yml:6-7,39-64`).

## Paths On The Server Port

| Path | Served By | Present When | Evidence |
|---|---|---|---|
| `/info` | `AppController`, marked `@Public()` and `@SkipThrottle()`. This is the deployment's health check target | Always — the controller is registered on the compiled module, outside any flavor check | `packages/backend/server/src/app.controller.ts:6-18`, `packages/backend/server/src/app.module.ts:158`, `render.yaml:15` |
...
| `/workspace/*path` | The doc renderer, reading prebuilt HTML from `static/` and `static/mobile` | Under the `renderer` or `front` flavor — the first of those also accepts `allinone` | `packages/backend/server/src/core/doc-renderer/controller.ts:54,66-68,80`, `packages/backend/server/src/app.module.ts:186` |
...
| Self-host app container → stores | Compose service names — `postgres:5432` in the connection string, `redis` as the key-value host, both on the default compose network | `.docker/selfhost/compose.yml:18-20` |
| Self-host migration job → stores | The same two addresses; the app container waits on it completing | `.docker/selfhost/compose.yml:8-14,30-32` |
| Render web service → stores | Injected at deploy time from the managed services; both stores set `ipAllowList: []`, which the file annotates as internal connections only | `render.yaml:27-40,46,52` |
| Render platform → web service | `GET /info` as the health check | `render.yaml:15` |
...
# health check reads (render.yaml:15), so a non-200 here is a failing deploy.
...
| The image advertises a port it does not listen on | `.github/deployment/node/Dockerfile:33`, `render.yaml:23-24` | The published image declares `EXPOSE 3010`; the deployment sets the process to bind `10000`. Any tool that reads image metadata to decide where to connect points at a closed port |
...
- The frontend dev server listens on `8080`. Nothing in the repository sets it;
  the number is inferred from the dev client's `webSocketURL`
  (`tools/cli/src/bundle-shared.ts:43`) and from the three client packages that
  point at `8080`. The bundler's default is a library fact, not a repository
  fact **(assumption — needs confirming)**.
...
- Which networks can reach a service marked `ipAllowList: []`
  (`render.yaml:46,52`) is decided by the hosting platform. The file's own
