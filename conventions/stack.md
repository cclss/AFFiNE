...
(`tools/utils/src/workspace.gen.ts:1459-1581`). The root is not one of them —
`@affine/monorepo` is absent from the file and from the `-p` validator, so every
count of 122 on this page excludes it while the `.` row above counts it.
`affine init` regenerates both from `yarn workspaces list` output
(`tools/cli/src/init.ts:83-97`, `tools/utils/src/yarn.ts:22-23`), and
`postinstall` runs `affine init` (`package.json:35`), so a clean install
rewrites it.
...
produced by `yarn install`. The one the server cannot start without is
`@affine/server-native`; `scripts/preview/build.sh:275-277` builds it as its
second step, and the image build compiles it in a stage of its own
(`.github/deployment/node/Dockerfile:198-203`). Nothing produces it at install
time — see [Known Gaps].
...
```sh
# Print the workspace members with their locations. This is the same call
# `affine init` uses to regenerate workspace.gen.ts
# (tools/utils/src/yarn.ts:22-23). It prints 123 lines — the 122 entries of
# PackageList plus the root, which `.` claims as a member (package.json:8) and
# which PackageList leaves out.
...
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Root `build` fails with no arguments                        | `package.json:24`, `tools/cli/src/build.ts:3`, `tools/cli/src/command.ts:40-44`, `preview.toml:36`            | `yarn build` expands to `yarn affine build`. `BuildCommand` extends `PackageCommand`, whose `--package,-p` is `required: true`, so the command errors before reaching `execute()`. There is no default target and no aggregate build across the 122 members. The repository does have a whole-tree build — `sh scripts/preview/build.sh`, which `preview.toml` declares — and the root script neither runs it nor names it                                                                                 |
| Root `dev` has no non-interactive path                      | `package.json:23`, `tools/cli/src/dev.ts:5`, `tools/cli/src/command.ts:106-133`, `tools/cli/src/affine.ts:29` | `yarn dev` expands to `yarn affine dev`. `DevCommand` extends `PackageSelectorCommand`, whose `-p` is optional; when it is absent `getPackage()` awaits an `inquirer` list prompt over the eight targets at `tools/cli/src/dev.ts:8-17`. With no TTY there is nobody to answer, and which of two failures follows depends on stdin: held open, the prompt never resolves and the command blocks; at EOF (`< /dev/null`), node exits 1 reporting `Detected unsettled top-level await`. Neither runs a target |
| The native addon the server loads has no install-time producer | `.yarnrc.yml:9`, `packages/backend/native/index.js:4-11`, `packages/backend/native/package.json:32`, `scripts/preview/build.sh:275-277` | `@affine/server-native` is a napi module compiled from Rust. With lifecycle scripts disabled nothing builds it during `yarn install`, and every server entry point — `yarn affine dev -p server`, `yarn run seed`, `yarn workspace @affine/server genconfig` — then exits with `Error: Cannot find module './server-native.x64.node'`. `yarn workspace @affine/server-native build` produces it; no error at install time says that it has to be run |
